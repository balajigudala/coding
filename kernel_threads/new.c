...
//static int __init fun_start(void)
{
	...
	pr_info("major no : %d\n",MAJOR(devno));
	pr_info("minor no : %d\n",MINOR(devno));
	res=register_chrdev_region(devno,1,"driver_mine");
	if(res<0)
	{
	...
	ptr=class_create(THIS_MODULE,"mydriverfolder1");
	dptr=device_create(ptr,NULL,devno,NULL,"mydevfile1");
//	proc=proc_mkdir("balu_dir",NULL);
//	proc_create("balu_file",0777,proc,&pops);
	thr=kthread_create(mykthread,NULL,"mykthread");
	if(thr)
	{
