...
//	res=register_chrdev_region(devno,2,"mydriver");
	res=alloc_chrdev_region(&devno,0,2,"newdriver");
	if(res<0)
	{
	...
	procc = proc_mkdir("myproc",NULL);
	proc_create("myproc_obser",0777,procc,NULL);


	printk(KERN_INFO"added sucessfully to kernel\n");
	ptr=class_create(THIS_MODULE,"newmydev");
